...
## Progress snapshot (as of 2026-04-29)

**Phase-a: code-complete; observability appendix #4 in flight as the active blocker for the `0.1.0-a` tag.** 32 / 32 PRs merged + 5 post-32 PRs landed. `main` is at commit `5a5b51e` (appendix #3 fix-up squash). The Management UI as merged is a config console, not a window into the running engine — the user's 2026-04-28 smoke session surfaced that an operator can't see agent runs unfold, the Sources STATUS column is cosmetic (`b.enabled ? "ok" : "paused"` only), there's no Review Dashboard UI consuming PR #28's server-side, no in-app Heartbeat reader, and no redaction-events surface. **Appendix #4 closes those gaps before tag.** See §1.2.12 below; full PR detail in `docs/plan-appendix/phase-a-4-observability.md`.
...
#### 1.2.12 Phase-a appendix #4 (pre-tag) — observability + Review Dashboard UI + Reports + badge legibility (post-40)

The 2026-04-28 smoke session against compose-spun production code surfaced that phase-a is engineered but not *observable*: operators can't see agent runs unfold, `agent_runs` / `llm_usage` data lives only in psql, the Sources STATUS column is hardcoded, PR #28's Review Dashboard server-side has no UI consuming it, and there's no in-app Heartbeat reader or redaction-events surface. The user (2026-04-28) decided this blocks the `0.1.0-a` tag — `0.1.0-a` should not ship "engine works, you can't see it work."

Five PRs (PR-A through PR-E) — full scoping in **`docs/plan-appendix/phase-a-4-observability.md`**. Pulls forward Heartbeat reader, Lint + Surfacer review UI (3 of 5 item types from phase-b PR #37), redaction-events surface (most of phase-b PR #39), and real-time streamed LLM tokens (was "v0.2 unless asked"). Only the in-app wiki page browser stays out (Gitea owns rendering by design). Estimated ~3–4 weeks; PR-A and PR-E land first (small, unblock visual confirmation), PR-B is the critical path (pioneers SSE + token streaming), PR-C and PR-D land in parallel after PR-B.
...
- [ ] THREAT-MODEL §5 PR checklist run on the phase-merge commit (commit hash updates to the appendix-4 closing squash before tag) — every box ticked or residual risk added to §7.
- [x] `CHANGES-v0.1.md` drafted with breaking-change list from pre-release to `a.N`. _(PR #38 / `e3a1da6`. Maintainer-edit before tagging; flagged human-follow-up items in the PR body. **Re-edit needed at appendix-4 close** to cover PR-A through PR-E.)_
...
- [ ] **Appendix #4 PR-A merged** — real source-binding status probe (3-state) + Sources table enrichment (`name`, `last_event_at`, `last_error`). See `docs/plan-appendix/phase-a-4-observability.md`.
- [ ] **Appendix #4 PR-B merged** — observability core (Activity feed + Run detail + Pipelines tab, first SSE route, streamed LLM tokens for in-flight runs gated by `LLM_DEBUG_LOG=1`).
- [ ] **Appendix #4 PR-C merged** — Review Dashboard UI for 3 of 5 item types (source-binding review, Lint findings, Surfacer candidates); skill candidates + marketplace updates ship later.
- [ ] **Appendix #4 PR-D merged** — Reports tab (Heartbeat reader + redaction events surface; redaction content never rendered per THREAT-MODEL §3.3).
- [ ] **Appendix #4 PR-E merged** — Badge legibility fix (`fontSize: 10 → 12`, status pills swap to glyph trio).
