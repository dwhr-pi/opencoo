...
## Progress snapshot (as of 2026-05-02)

**Phase-a: `0.1.0-a` shipped 2026-05-01; appendices #5 (production scheduling), #6 (pilot autonomy), and #7 (scheduled agents do their jobs) all complete.** Core 32 / 32 PRs + 5 post-32 + 1 appendix-#4 scoping (#41) + 10 appendix-#4 PRs (#42–#51) + 3 appendix-#5 PRs (#53, #54, #55) + 3 appendix-#6 PRs (#56, #57, #58) + 3 appendix-#7 PRs (O1 `ec3efb2` / #59, O3 `951aae7` / #60, O2 `26126f1` / #61). After appendix #7: `wiki://{slug}/{path}` MCP resources registered alongside `worldview://{slug}` so Heartbeat / Lint actually read pages instead of `McpResourceNotFoundError`-ing on every dispatch; `opencoo agents fire <slug>` CLI gives operators a manual-trigger verb; Surfacer activates by default using either n8n-mcp's `search_templates` (when `N8N_MCP_*` env set) or the vendored `builderSkills` baseline (3 slugs) as floor. Full appendix-#4 PR detail in `docs/plan-appendix/phase-a-4-observability.md`; appendix-#5 (M1/M2/M3) in §1.2.13; appendix-#6 (N1/N2/N3) in §1.2.14; appendix-#7 (O1/O2/O3) in §1.2.15 below.
