...
- per-provider keys (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `OLLAMA_BASE_URL`, `OPENROUTER_API_KEY`) — wired by the `LlmRouter` lazily; only required for the providers the operator actually selects in the management UI's LLM-policy editor (per-tier provider + model dropdowns; `ollama` accepts a custom-input model since the operator pulls those locally). (`OPENROUTER_API_KEY` is documented in the table above; the others are infrastructure config and follow the same lazy-resolve pattern.)
...
1. **Disable the binding.** Sources tab → click the binding row → the drill-down modal opens (PR-Q10) → click **Disable** → confirm. The PATCH is CSRF-gated and writes a `source_binding.update` audit row. inbound webhook events continue to be accepted (HMAC verified, `webhook_events` row written) but the scanner queue does not dispatch them, so no compile fires and no wiki write happens. this preserves audit and lets the operator inspect what would have ingested without commit. (For permanent removal — typically only on a fresh binding the operator just misconfigured — the same modal exposes **Delete**, which clears the binding plus its `webhook_events` and `ingestion_intake` history in one transaction; append-only audit such as `redaction_events` is preserved and a 409 surfaces if the binding has any of that history. Disable is the safer default during cutover.)
...
One step is required after every pull of upstream changes — the schema-migration step is now automated by default at engine boot (PR-X1, phase-a follow-up); the UI bundle rebuild remains a manual step.

### 11.1 Schema migration

> **Optional. The engine auto-applies migrations at boot.** If you set `OPENCOO_AUTO_MIGRATE=0` in `.env` to keep the legacy manual flow, this step is REQUIRED — otherwise it's a safe no-op (the engine's auto-migrate run will see migrations are already applied and exit fast).

PR-X1 (phase-a follow-up) lifted the v0.1 deferral: `engine-self-operating`'s `start()` now calls `applyMigrationsWithLock` from `@opencoo/shared/db` BEFORE Fastify binds the listener. The same helper is what the `opencoo migrate` CLI verb runs, so the operator-driven flow and the boot-driven flow share one code path. A process-wide `pg_advisory_xact_lock` keyed on `hashtext('opencoo.auto_migrate')::bigint` serialises concurrent migrators (an operator running `opencoo migrate` at the same moment an engine boots is safe — one waits, the other becomes a no-op). Drizzle tracks applied migrations in `drizzle.__drizzle_migrations`, so a re-run on an up-to-date schema is a no-op (`migrate.applied` log line with `durationMs` near zero).
