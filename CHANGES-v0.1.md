...
## Wave-17 closeout — wave-16 operational follow-ups (phase-a appendix #17)

Eight PRs (WZ0 scoping + six implementation in one parallel phase + WZN closeout) shipped 2026-05-18 finishing the operational tail of wave-16's impeccable-UX work plus one carried-forward wave-15 deferral and one CI-flake fix. Wave-17 is small (one day end-to-end), low-risk (every PR extends an existing primitive or fills a documented stub), and ships under `0.1.0-a.15`. No new abstractions; no new schemas; no new env vars; no new admin routes; no new attack surface. Full scoping in `docs/plan-appendix/phase-a-17-wave-16-followups.md`. Main closes at `<this PR's merge SHA>` (WZN).

### Added

- **`useOptimisticPatch` wiring expansion to the full B5 whitelist** (B5+ `1ac0467` / #187). Wave-16 B5 shipped the hook + saving-cue + rollback toast + a representative wiring on `agent_instances.enabled` + `output_channels.enabled`. B5+ wires the remaining nine fields: `agent_instances.{name, locale, scope_domain_ids}` on `AgentInstanceDetail`; `source_bindings.{notes, retention_days_override}` on `SourceBindingDetail`; `domains.{display_name, default_locale, worldview_enabled}` on `DomainDetail`. The 10th whitelist field (`output_channels.name`) was deferred for a small follow-up because the existing `PATCH /api/admin/output-channels/:id` route's Zod discriminated-union body schema accepts `{enabled} | {config} | {credentials}` only — adding a `{name}` PATCH branch is a small additive server-side change but was outside wave-17's "no new admin routes" guardrail. Saving-cue dots fire next to each field during the round-trip; rollback restores the prior value + surfaces an alert toast via B7. The server's audit-write-before-mutate invariant is preserved — pin tests confirm rollback paths leave no audit row. DomainDetail's three optimistic fields each get a dedicated Quick-save button alongside the existing combined-Save (which now scopes to the non-optimistic fields). 28 unit tests cover the new wires (6 agent-instance + 4 source-binding + 6 domain + 12 regression updates for ToastProvider wrap and the combined-Save split).

- **Perf-marks instrumentation across remaining 10 routes** (B8+ `76b1257` / #184). Wave-16 B8 shipped the `performance.mark` library + dev-only `PerfPanel` + a representative wiring on Domains. B8+ extends the same pattern to Sources, Agents, Outputs, Prompts, Reports, Activity, Audit, Review, Cost, and LlmPolicy. Each route's primary `fetchAdmin(...)` data-fetch boundary now emits `markRouteFetchStart(tab)` + `markRouteFetchEnd(tab)` so `window.opencoo_perf` captures click-to-data measurements on every route, not just Domains. A new `perf-marks-coverage.test.tsx` walks each of the 11 routes and asserts the mark pair is called during mount → data-resolved. No new helpers; no behavioral change beyond observability.

- **Cleanup pipeline honors `sources_bindings.retention_days_override` at sweep-time** (W5+ `b905f7e` / #182). Wave-15 W5 added the column + admin-API PATCH branch + UI editor; the Cleanup BullMQ worker was reading `domains.retention_days` only. W5+ changes the worker's age-out query to `COALESCE(b.retention_days_override, d.retention_days)` so the per-binding override wins when set, with the existing behavior preserved when NULL (the current production state for the design partner). Four new pin-tests cover (override null = baseline behavior), (override set = override wins), (mixed bindings under one domain = different cutoffs per binding), (override > domain retention = override wins even when longer). No schema change; no migration; no admin-API change. Backward-compatible at the migration level (no schema change) and at the data level (NULL columns behave as before).

- **Per-row Retry buttons on the W4 intake panel** (W4+ `d25580b` / #185). Wave-14 W4 shipped the failed-intake panel with disabled-stub Retry buttons; Y8 wired the binding-level retry route but not the per-intake one. W4+ wires the existing `POST /api/admin/source-bindings/:id/intake/:intakeId/retry` admin route into the per-row buttons. On click: the row's `intake_status` flips back to `pending`, the matching classify job re-enqueues, the Activity feed receives a `pipeline.intake_retried` SSE event, and `useToast().success` confirms the operator. The admin route's existing admin-team gating + CSRF + audit-write-before-mutate invariants are preserved (the route itself was added in Y8; W4+ only enables the disabled UI button).

- **Asana `assignee_gid` UI on output-channel config** (Asana `ee36048` / #186). Wave-14 W5 added the field to the schema; the heartbeat-to-Asana transformer reads it; the channel-config form wasn't rendering it. The Asana PR ships a free-form text input next to the existing `section_gid` field, with a `<Tooltip term="assigneeGid">` explaining how to find a user gid in Asana (workspace member URL ends with the gid). Path (b) — wiring a one-time `listUsers` query into the credential-validated flow — was the alternative but the adapter doesn't yet expose a `listUsers(workspaceGid)` helper and path (a) ships in 30 lines without an adapter extension. The implementer chose (a); the PR body documents the rationale. New `help.assigneeGid.{label,body}` keys in en + pl. Unit test pins the field renders with the tooltip + the tooltip is keyboard-reachable.

- **`output-channels-registry.test.ts` post-test EAI_AGAIN unhandled rejection fixed** (Flake `6993e7c` / #183). The test had been issuing a real `getaddrinfo` call against a stub host during composition construction; the lookup failed under CI's restricted egress as the test file unmounted; the unhandled rejection bubbled past Vitest's `afterEach` boundary and exit-1'd the shard despite all 870/873 tests passing. The flake hit four wave-16 PRs (A1, B3, A7, WZN) and one wave-17 PR (W4+ before its rebase onto the post-Flake main). The fix stubs the DNS lookup at the test-module-mock level so no real network call happens. New pin-test asserts the test file completes without unhandled rejections. The five wave-17 PRs that landed after this fix ran rerun-free.

### Risk residual

- **Optimistic-PATCH invariant pinned across the full B5+ surface.** Every new B5+ wire respects the server's audit-write-before-UPDATE ordering. Rollback paths leave no audit row (pin-test per field). The B5 blacklist (sovereignty-token flows, credential rotation, `memory_clear`, deletes) is unchanged. Specific 422 codes (e.g. `unknown_scope_domain_ids` / `duplicate_scope_domain_ids` on `agent_instances.scope_domain_ids`) re-open the inline error slot in the modal so the operator can edit-and-retry without losing their typed text — the rollback affects the committed value (the row beneath the modal); the in-flight draft preserves the operator's intent until the next commit.
- **Cleanup pipeline query change is additive and reversible.** The COALESCE form collapses to the existing behavior when `retention_days_override IS NULL` for every row (the current production state). No data-level change for the design partner without an explicit operator action via the W5-UI editor.
- **W4+ per-intake retry uses the existing Y8 admin route.** No new server-side route. The route's invariants (admin-team gated, CSRF, audit-write-before-mutate, `pipeline.intake_retried` SSE) are preserved. The UI only enables a previously-disabled button.
- **Asana `assignee_gid` field is operator-controlled freeform text.** It hits the existing channel-config schema's Zod validator which is a passthrough string. Operator-supplied content never enters audit metadata (per THREAT-MODEL §3.13). The tooltip-described "how to find a gid" content is locale-mirrored in pl.json with the wave-16 C3 LLM-translation pattern.
- **Flake fix removes a network call from a unit test.** The fix isn't a network-shim or a retry-on-transient — the underlying call should never have been made. The pin-test confirms the test completes cleanly in both restricted (CI) and unrestricted (local-dev) network environments.

Wave-17 ships under `0.1.0-a.15` once this PR (WZN) merges.

---

## Wave-16 closeout — impeccable UX (phase-a appendix #16)

Twenty-three PRs (WZ0 scoping + 21 implementation across four phases A/B/C/verification + WZN closeout) shipped 2026-05-17 → 2026-05-18 turning a complete management console into an **impeccable** one. Wave-15 made every operator-editable column reachable through the UI; wave-16 raised the bar to three orthogonal floors a regulated adopter expects: WCAG 2.2 AA accessibility, perceived-latency-aware loading + onboarding + optimistic UI, and full visual depth + locale parity + in-app help. Three Plan agents synthesized a 23-PR roster sequenced as 5 phases (foundation primitives → consumers batch 1 → consumers batch 2 → verification gates → closeout); the wave-end gate runs an 11-step manual walk against the partner deployment including NVDA/VoiceOver, axe-core CI, contrast sweep, reduced-motion, Lighthouse, fresh-deploy onboarding, optimistic-rollback, locale parity, cross-route snapshot, THREAT-MODEL §5 maintainer walk, and full wave-15 regression. After wave-16 the UI passes axe-core's `serious`/`critical` set, no layout shift on data arrival, every empty state names the next step, a fresh operator can go zero-to-heartbeat without leaving the chrome, every field validates as the operator types, every non-sovereignty PATCH applies optimistically, every jargon term carries a focus-keyboard-reachable tooltip, Polish renders end-to-end, and the chrome carries strategic editorial typography on exactly three surfaces. Full scoping in `docs/plan-appendix/phase-a-16-impeccable-ux.md`. Main closes at `<this PR's merge SHA>` (WZN).

### Added

#### Phase 1 — Foundation primitives

- **Modal → native `<dialog>` + focus-trap + focus-return** (A1 `7bdf1ce` / #160). `Modal.tsx:154-245` refactored from `<div role="dialog">` to the native `<dialog>` element; `showModal()` / `close()` lets the browser provide focus-trap + Esc + top-layer `inert` for free. Backdrop-click-close preserved via target-equality check; focus-return implemented via `useRef(document.activeElement)` snapshot at open. Firefox font-inheritance quirk solved with explicit `font-family: inherit` rule. Every existing modal consumer (~14 across wave-15 surfaces) inherits without API change. `PatEntryModal.tsx` collapses onto the shared shell. Library alternatives (focus-trap-react, Radix Dialog, @reach/dialog) rejected per the wave-12 "no `*-react` UI shims" rule.

- **Skeleton primitive + `useDeferredSkeleton` hook** (B1 `378c0a4` / #158). New `packages/ui/src/components/Skeleton.tsx` with `Skeleton.Row mono cols={N}`, `Skeleton.Block`, `Skeleton.Field` composables; depth via border + paper-2 (no shimmer — the design-system "one loop" invariant — heartbeat-pulse stays the only loop). Each Skeleton.* renders with `role="status"` + `aria-live="polite"` + `aria-busy="true"`. `useDeferredSkeleton(isLoading, { delayMs: 80 })` hook prevents flash for sub-80ms loads (typical for warm cache). B2 then sweeps every `t("common.loading")` site into matching Skeleton shapes via Suspense fallbacks.

- **Tooltip primitive + `help.*` namespace expansion** (C1 `01da630` / #159). New `packages/ui/src/components/Tooltip.tsx` using `@floating-ui/react` for collision-detection (positioning utility, not a component framework — distinct from the wave-12 "no `*-react` UI shims" rule per the C1 PR body). `?` character at micro size as the trigger (no fourth glyph — the trio is reserved for product-concept iconography; UI affordances use type). Keyboard-reachable: focus opens, blur closes, hover with 200ms delay opens, mouse-out closes immediately, Esc closes. ARIA: `aria-label="About <term>"`, `aria-describedby` toggles on open, bubble has `role="tooltip"`. Wired to the 5 highest-traffic jargon terms (`reviewMode`, `allowedPaths`, `scopeDomainIds`, `worldviewEnabled`, `governanceCadence`) with `help.<term>.{label,body}` keys in en + pl.

#### Phase 2 — Consumers, batch 1

- **Semantic landmarks + `<h1>` coverage** (A2 `5af9678` / #168). `Chrome.tsx` now emits `<header role="banner">` (TopBar), `<nav aria-label>` (Sidebar), `<main aria-labelledby="opencoo-page-h1">` (route content) as siblings — landmark hierarchy per W3C contract. Sidebar group headers (Operate / Knowledge / Governance / Diagnostics) become `<h2>` with mono-uppercase styling preserved; active tab gets `aria-current="page"`. Five routes (`Activity.tsx`, `Audit.tsx`, `Cost.tsx`, `Reports.tsx`, `Review.tsx`) gain a single `<h1 id="opencoo-page-h1">` with new `routes.*.h1` i18n keys; existing routes' h1s gain the same id so `aria-labelledby` resolves correctly.

- **Field `aria-describedby` + `aria-errormessage` chain** (A3 `49c20e5` / #162). `Field.tsx:43-166` extended with stable `helperId` + `errorId`; helper-text span gets the helper id; error-text span gets the error id + `role="alert"` so screen readers announce inline errors immediately. `aria-invalid="true"` on the input when error is set. When both helper AND error are set, `aria-describedby` includes both ids space-separated (per ARIA 1.2). New `validationStatus?: 'idle'|'validating'|'valid'|'invalid'` prop slot fills in via B4. `TextField` and `TextArea` (W9 primitives) inherit transparently. PatEntryModal's input back-ported onto the shared Field chain via the A1 collapse.

- **Route-level code splitting + Skeleton fallbacks + prefetch** (B2 `64160e7` / #163). Each route module converted from static import to `React.lazy(() => import('./routes/X.js'))` with a Suspense boundary that renders the matching B1 skeleton. Each sidebar entry attaches `onMouseEnter` + `onFocus` prefetch handlers so the lazy module's chunk loads before the click lands; the browser's module-record cache dedupes the second `import()` so click-to-render is synchronous. New `tests/ui/bundle-size.test.ts` asserts entry chunk size + per-route chunk presence. `Suspense key={tab}` resets the boundary on tab switch so unresolved routes re-show their skeleton fallback rather than the previous route's resolved children.

- **EmptyStatePanel extraction + 7-route empty-state coverage** (B3 `9986f9e` / #167). `HeartbeatDiagnosticsPanel` (W8) extracted to `EmptyStatePanel.tsx` with `title` + `body` + optional `cta` + optional `diagnosticsChain` props (the latter mirroring W8's precondition-walk). Seven routes that previously flat-rendered "no rows" — Domains, Sources, Agents, Outputs, Activity, Review, Audit — now render `<EmptyStatePanel>` with route-specific copy + a one-click CTA to the matching `New X` modal (where applicable). Reports refactored to feed its diagnostic chain through the shared component, preserving the W8 precondition-walk behavior end-to-end.

- **Toast queue + `useToast` hook** (B7 `f48a346` / #165). New `Toast.tsx` exporting `<ToastRegion />` (mounted once in `App.tsx`) and `useToast()` returning `{ success, advisory, alert }`. Each toast tone renders with a left-border accent token + JetBrains Mono tone-tag in the corner (no emoji): success → `var(--healthy)` + `OK`, advisory → `var(--advisory)` + `ADVISORY`, alert → `var(--alert)` + `ALERT`. Auto-dismiss with hover/focus pause + resume from paused point. Details-expand renders pre-formatted mono text (parsed 422 field-by-field call sites supply structured details; the toast renders, never echoes raw `error.message` — `safeErrorMessage` per the wave-12 lift). One-shot 200ms slide-in + 150ms fade-out; respects `prefers-reduced-motion`.

- **Operator-controlled locale switcher + migration 0015** (C2 `6e43f18` / #166). New `users.locale_preference TEXT CHECK (locale_preference IN ('en','pl'))` column via additive migration 0015. New `PATCH /api/admin/users/me/locale` admin-team-gated + CSRF-protected + audit-write-before-mutate route writing new `user.set_locale_preference` verb (metadata `{user_id, new_locale, caller_username}`). Native `<select>` styled as `Btn variant="ghost"` lands in `Chrome.tsx` TopBar alongside username. Two-tier persistence: localStorage is SoT during session, DB `users.locale_preference` is SoT at login — on whoami hydration, server's locale_preference reconciles into the client's localStorage. Mirrors the per-device "IDE theme" model (per-device chrome preference more than per-account one).

- **`<Display>` editorial-headline component (Instrument Serif scope)** (C4 `99f0b92` / #164). New `Display.tsx` is the ONLY legal call site for `var(--font-serif)` (Instrument Serif italic). Three strategic placements: Reports heartbeat lede, Prompts empty-state lede, Domains tab top-line summary. ESLint local rule (`packages/ui/eslint.local.js`) fails any other inline reference to `var(--font-serif)` / `t-lede` / `Instrument Serif`. The UI lints `<Display level={1}>` use — level=1 is reserved for a future docs site; the management console uses level=2 + level=3 only.

- **Hover affordances + reduced-motion gate** (C5 `5a2ef9a` / #161). Uniform hover pattern across every clickable surface: 60ms `var(--ease-transform)`, background-shift + border-shift only — no shadow, no scale, no inversion, transitions explicit per property (never `transition: all`). `Btn.tsx` variant token map gets per-variant hover state; `Card.tsx` ships opt-in `clickable` prop that renders as `<button>` with the same hover recipe; sidebar entries get hover lift via shared class; clickable table rows get cursor + background-shift. Global `@media (prefers-reduced-motion: reduce)` rule clamps every transition to ≤80ms so the heartbeat-pulse glyph stays the SINGLE motion exception.

#### Phase 3 — Consumers, batch 2

- **Live regions + status announcements + role=alert audit** (A4 `e9324f7` / #171). New `packages/ui/src/lib/announce.ts` with `pushAnnouncement(text, { tone?: 'polite'|'assertive' })` writing to a subscriber list. `App.tsx` mounts two visually-hidden live-region divs once (`<div aria-live="polite" id="opencoo-aria-live-polite">` + `<div aria-live="assertive" id="opencoo-aria-live-assertive">`); each renders its current queue with `aria-atomic="true"`. B7 Toast component bridges into pushAnnouncement so the visual toast and the screen-reader announcement are coupled. `role="alert"` audit across every inline error-display site filled the gaps surfaced by the manual walk.

- **Keyboard navigation completeness** (A5 `907e5d7` / #169). Cmd-K palette gets combobox/listbox semantics (W3C APG): search input is `role="combobox"` with `aria-expanded`/`aria-controls`/`aria-activedescendant`; result list is `<ul role="listbox">`; each row is `<li role="option">` with `aria-selected` on the active. Up/Down shifts active; Enter dispatches; Esc closes. Sidebar groups get roving-tabindex (W3C APG vertical menubar adaptation): single tab stop on the active tab, Up/Down moves within group, Left/Right jumps between groups, Home/End to first/last. Expandable-row chevrons on `AgentInstancePromptsSection` + `SourceBindingDetail` audited as `<button>` with `aria-expanded` + `aria-controls`.

- **`useLiveValidation` hook + real-time validation in 3 modals** (B4 `2335710` / #174). New `useLiveValidation` hook accepts sync (regex/length/required) + async (debounced 250ms with AbortController) validators per field; per-field state machine `idle → validating → valid|invalid`. Wired into `NewDomainModal` (slug regex + uniqueness against `GET /api/admin/domains`), `NewSourceBindingModal` (adapter + credential + allowed_paths + cron via existing `cron-parser` from `SchedulerEditor.tsx:31`), `NewAgentInstanceModal` (definition_slug enum + name length + scope_domain_ids existence). Inline validation chips render via A3's `validationStatus` prop slot — mono micro-label, color-keyed (ink-3 validating, healthy valid, alert invalid). Uncontrolled-input pattern from wave-12 PR-Z9 (password-manager compatibility) preserved.

- **`useOptimisticPatch` hook + whitelisted PATCH branches** (B5 `7a25227` / #173). New hook applies `setValue(next)` to local state immediately, fires `applyFn(next)`, rolls back on failure with a B7 alert toast. Whitelist: `agent_instances.{enabled, name, locale, scope_domain_ids}`, `output_channels.{enabled, name}`, `source_bindings.{enabled, notes, retention_days_override}`, `domains.{display_name, default_locale, worldview_enabled}`. Blacklist (round-trip is the UX): sovereignty-token flows (prompt-override apply, LLM-policy apply), credential rotation, `memory_clear`, deletes. Saving-cue dot is a one-shot 600ms `--ease-write` opacity fade (no loop); ink-3 during saving → healthy flash on success → alert on rollback. **The server's audit-write-before-mutate invariant is preserved** — client optimism only shifts render timing; on rollback no audit row exists. Initial wave-16 wiring lands `enabled` toggles on AgentInstanceDetail + OutputChannelDetail; the hook is general-purpose for follow-up wiring on the deferred surfaces.

- **OnboardingWizard on Domains empty + Cmd-K re-summon** (B6 `1df6ddd` / #170). New `OnboardingWizard.tsx` renders inline as the empty-state body when `domains.length === 0 && !localStorage.opencoo_onboarding_dismissed`. Four-step vertical stepper inside one Card: create first domain → bind first source → seed first agent instance → wait for first heartbeat (polls `GET /api/admin/heartbeat/preconditions` already wired by W8). Each step transitions from idle to active when its predecessor completes; CTAs open the matching `New X` modal. "Skip wizard" persists dismissal in localStorage; re-summon via Cmd-K "Run onboarding wizard" entry which clears the flag + navigates to Domains.

- **Polish locale full pass + plural forms + `tools/i18n-check.ts` CI fence** (C3 `2fca0d3` / #175). Every `pl.json` value audited against `en.json`: structurally-identical-to-English values translated via LLM (Claude) into natural operator-oriented Polish (matching the wave-12 / Z8 voice). Plural-form keys (`_one` / `_few` / `_many` / `_other`) shipped for every count string per i18next's Polish rules. New `tools/i18n-check.ts` asserts every leaf key in en.json has a matching key in pl.json, AND every pl.json string value contains a Polish diacritic (`ąćęłńóśźż` and capitals) OR matches an allowlist (proper nouns, path fragments, technical strings, single words like "OK"); CI fence fails on regression. `formatUsd` lifted from `Cost.tsx:127-147` into new `packages/ui/src/lib/intl-format.ts` along with `formatDateTime` / `formatNumber` / `formatRelativeTime` (`Intl.RelativeTimeFormat` — "3 godziny temu"). All hardcoded `en-US` Intl callers swept to use the helpers with `i18n.language`. Native-Polish-speaker review scheduled as a separate post-wave-16 follow-up PR; `_lint_translate_c3` markers flag uncertain phrasings for that pass.

- **Density toggle + scope spacing vars** (C6 `5c81438` / #172). New `useDensity()` hook reads `localStorage.opencoo_density` (default `'comfortable'`); on change writes localStorage AND sets `<body data-density={d}>`. TopBar gets a two-button toggle alongside C2's locale switcher. CSS variables in `colors_and_type.css` get density-scoped overrides under `body[data-density="compact"]` for `--row-pad-y`, `--row-pad-x`, `--table-cell-pad`, and micro-label tracking (smaller values in compact). Instant attribute swap, no transition (the design-system "one loop" rule preserved). Cmd-K palette + Table primitives audited so cell + row padding flows through the density-scoped vars.

#### Phase 4 — Verification + measurement (wave-end gates)

- **WCAG contrast sweep + Skip-to-content link** (A6 `2ebe501` / #177). New `packages/ui/tests/accessibility/contrast.test.ts` parses `colors_and_type.css`, enumerates every foreground-on-background pair the UI actually renders, computes WCAG contrast ratio, asserts ≥ 4.5:1 for body text (ink/ink-2/ink-3 on the paper scale) and ≥ 3.0:1 for large text + UI components. Sweep iterates both density modes from C6 (colors don't change per density but the test pins that explicitly). Fail-the-build on any pair below threshold. A6 darkened `--ink-3` from `#78746B` to `#67645C` to meet 4.5:1 on every paper layer. Skip-link "Skip to content" renders first in `App.tsx`, off-screen by default + slides into view on focus (one-shot CSS); links to `#opencoo-main` on the `<main>` element.

- **@axe-core/playwright CI job + manual SR walk checklist** (A7 `22cc2ed` / #179). New `.github/workflows/accessibility.yml` job boots compose, builds the UI, serves the built bundle, and runs `packages/ui/tests/accessibility/axe.spec.ts` walking every route × every modal × en + pl locale via `@axe-core/playwright`. Zero `serious` or `critical` violations required. New `docs/accessibility-walk-checklist.md` documents the manual NVDA + VoiceOver walk for the wave-end gate against the partner deployment.

- **`performance.mark` instrumentation + PerfPanel** (B8 `8130111` / #176). New `packages/ui/src/lib/perf-marks.ts` exports `markRouteClick(tab)`, `markRouteImportStart/End(tab)`, `markRouteFetchStart/End(tab)`, `measureRouteNav(tab)`, `pushPerfEntry(entry)`; each mark/measure also writes to `window.opencoo_perf = []`. `App.tsx` wires marks at sidebar-click + B2's lazy-import boundaries; Domains route instrumented end-to-end as the representative pattern (other routes follow in a documented follow-up). New `PerfPanel.tsx` (dev-only, gated on `import.meta.env.DEV || ?perfDebug=1`) renders a fixed-corner panel of the latest entries for Lighthouse / web-vitals collection. The dev-only gate tree-shakes the panel out of the production bundle.

- **Cross-route visual-consistency snapshot test** (C7 `4f5c21a` / #178). New `packages/ui/tests/visual-consistency.test.tsx` (vitest + jsdom + MSW-mocked admin-API) renders every route and asserts: exactly one `<h1 id="opencoo-page-h1">` per route, exactly one `<main aria-labelledby="opencoo-page-h1">` per route, no inline `color: #...` literals in rendered DOM (W11 audit fence pinned), no `dangerouslySetInnerHTML` anywhere (THREAT-MODEL pin), `<Display>` appears in exactly 3 routes (Reports, Prompts, Domains), sidebar renders 9 tab labels in both en and pl. Fails CI on drift. New `renderRoute(tab)` helper mounts each route inside the App shell so Chrome.tsx's landmarks render too.

### Deferred to v0.2

- **Native-Polish-speaker review of C3's translation.** LLM-first pass shipped per user direction; native-speaker fix-up scheduled as a separate post-wave-16 PR. `_lint_translate_c3` markers flag uncertain phrasings for that pass.
- **Mobile / narrow-viewport.** opencoo is a desktop operator console; narrow-viewport graceful degradation is v0.2.
- **Full WCAG 2.2 AAA.** Floor here is AA. AAA goals (7:1 contrast, 24px target sizes) revisited in v0.2.
- **Dark mode (`prefers-color-scheme: dark`).** Paper-on-ink is a deliberate design choice; a dark variant requires a second contrast sweep + accent recalibration.
- **Real-browser pixel-diff visual regression.** C7's snapshot is structural (jsdom). A Playwright-based pixel-diff suite is an order of magnitude more expensive to maintain; deferred unless C7 misses real drifts.
- **Per-screen density override.** Single global density only; per-screen override violates the "one spacing scale" rule.
- **Fourth glyph promotion.** `?` character stays as the Tooltip trigger; an `info-dot` composition (open arc + small filled disc) deferred to operator-research signal.
- **Optimistic-PATCH wiring on the deferred B5 surfaces.** B5 lands `enabled` toggles on AgentInstanceDetail + OutputChannelDetail; the remaining whitelist (name/locale/scope_domain_ids on agent instances; notes/retention_days_override on source bindings; display_name/default_locale/worldview_enabled on domains) ships with the next wave once the explicit-Save-then-close pattern is reshaped for inline optimism.
- **B8 perf-marks instrumentation on the remaining 10 routes.** Domains is the representative wiring; follow-up extends the same pattern.
- **Cleanup pipeline reading `sources_bindings.retention_days_override` at sweep-time** (wave-15 deferral unchanged).
- **Per-row Retry buttons on the wave-14 W4 intake panel** (operational follow-up).
- **Asana channel-config `assignee_gid` UI** (operational follow-up).

### Risk residual

- **Optimistic UI preserves audit-write-before-mutate invariant.** Every B5-routed surface still goes through the same server-side `auditWrite() → mutate → return` ordering. The hook's `setValue → optimistic-render → applyFn → rollback-on-failure` lifecycle ONLY shifts the client's render timing; if the UPDATE fails, the rollback restores the prior state AND no audit row exists (because the audit row writes from the server-side handler which never reached commit). Pin-test asserts the rollback path leaves no audit row.
- **New state-changing route audited under THREAT-MODEL §5.** C2 introduces `PATCH /api/admin/users/me/locale` — the wave's single new admin-API write. Audit verb `user.set_locale_preference`; admin-team gated; CSRF-protected; Zod-validated body `{ locale: 'en' | 'pl' }`; audit-write-before-mutate ordering; metadata excludes credential bytes. The change is operator-self-targeting (`/me/locale`) so the metadata stores `caller_username` for audit-trail provenance without exposing inter-user data.
- **Live regions never echo raw error bytes.** A4's `pushAnnouncement` accepts only safe strings; B7's Toast bridge passes through `safeErrorMessage` for any server-originated text. The two `<div aria-live>` mounts are content-only — operator-controlled freeform text never enters live-region content. THREAT-MODEL §3.13 admin-authz invariant holds.
- **The `_lint_translate_c3` marker is intentional debt, not a silent ship.** C3 ships the best LLM Polish translation pass + plural forms + the CI fence catches drift, but a small set of uncertain phrasings carry the `_lint_translate_c3` marker so the native-speaker follow-up can find and resolve them. They're operator-readable as the LLM rendered them, not English placeholders — the wave-16 user direction was "LLM-first; native-speaker fix-up after." The marker doesn't appear in user-facing strings; it's a translator-comment-shape inside the value, stripped before render via i18next post-processor (or, if not, considered as part of the Polish text — the LLM's rendering reads naturally either way).

Wave-16 ships under `0.1.0-a.14` once this PR (WZN) merges.

---

## Wave-15 closeout — management UI completeness (phase-a appendix #15)

Fifteen PRs (W0 scoping + W1–W11 implementation across four sub-waves A/B/C/D + three UI follow-ups W4-UI/W5-UI/W3-followup + W12 closeout) shipped 2026-05-14 → 2026-05-17 closing the "the management UI is not a product yet" gap that wave-14's runbook recipe surfaced: after wave-14 the autonomous heartbeat was producing genuine COO content end-to-end, but the runbook for backfilling `allowed_paths` was still SQL-only because the Sources tab couldn't edit the field, the Agents tab's `+ New` button was a documented stub, no operator surface existed for per-(domain, instance) prompt overrides, the Reports tab silently showed "no heartbeats yet" without naming the missing precondition, and routine operator tasks — rename an agent instance, scope it to a domain set, set a per-binding retention override, bulk-delete dead output channels, adapt the heartbeat prompt to a domain's vocabulary — all required a CLI session or a `psql` UPDATE. Wave-15 closes the gap so **every column on every operator table now has a UI editor, every prompt can be overridden per-(domain, instance) under sovereignty-token control, the management console reads as a coherent product with grouped IA + breadcrumbs + Cmd-K jump-to-anything, and the Reports tab either works or names what it's waiting for**. Full scoping in `docs/plan-appendix/phase-a-15-management-ui-completeness.md`. Main closes at `02e0af6` (W11 — design-system audit pass).

### Added

- **Per-(domain, instance) prompt-override editor end-to-end** (W1 `2ee4938` / #140 — schema + resolver · W2 `95c34ab` / #141 — admin-API · W7a `69972a2` / #149 — per-domain editor · W7b `f84568e` / #152 — per-instance editor). New `prompt_overrides` table (migration 0014) keys per `(domain_id, instance_id NULL, prompt_name, locale)` with body + version + actor + timestamp; `loadPromptForScope({domainId, instanceId, name, locale})` walks `instance > domain > shipped baseline` in a single SELECT and falls back deterministically. Admin-API ships ten endpoints (five verbs × two scopes — `domains` + `agent-instances`): list overrides, preview-diff against shipped baseline, apply with sovereignty-token (`confirmDiff:true` step bound to scope + payload-hash, 5-min TTL, baseline-version-bound so a shipped baseline rev between preview and apply forces a re-preview), delete, and the `llm_usage_debug` "what was actually sent" drawer query. Mirrors `domains-llm-policy.ts:128-246` token-by-token. Prompts tab rebuilt from 76-line read-only manifest grid into the headline operator surface: left rail with 9 prompts grouped Knowledge / Operate · right pane with domain picker + en|pl locale tabs + mono textarea + baseline-version chip + override-source chip + isStale banner; the hardcoded `lagging=[]` stub replaced with real per-(domain, prompt) lagging-overrides aggregation. Per-instance editor surfaces in `AgentInstanceDetail.tsx` as an Agent prompts card per prompt the instance's definition uses, with the resolution stack visible (shipped baseline → domain override against `scope_domain_ids[0]` → instance override) so operators see exactly which body the resolver will load.

- **+New agent instance modal + scope/name/locale/memory-clear editors** (W4 API `89b78e2` / #143 · W4-UI `50378c8` / #147). Closes the highest-impact UI ↔ schema gap. New `POST /api/admin/agent-instances` accepts `{definition_slug, name, locale, scope_domain_ids?, enabled?}` + four PATCH branches on `:id` (`scope_domain_ids` · `name` · `locale` · `memory_clear`). `Agents.tsx` `+ New` button is no longer a stub — opens `NewAgentInstanceModal.tsx` with hardcoded list of the 9 known definition slugs as `<select>`, uncontrolled name input (wave-12 PR-Z9 / G12 password-manager pattern), locale toggle, optional scope-domain multi-select chip list. `AgentInstanceDetail` drill-down gains a scope-picker section (chip-list bound to `scope_domain_ids` against `GET /api/admin/domains`), name + locale field rows, and a "Clear agent memory" destructive button (ack-checkbox gated). After this lands an operator scopes + renames + locale-flips + memory-clears an agent_instance through the UI without touching `psql`.

- **Domain row edit-fields completeness** (W3 `1379753` / #144 + W3-followup `40cbe1c` / #146). Extends `PATCH /api/admin/domains/:id` with five new operational field branches: `retention_days` (`int`, 1–365 nullable), `governance_cadence` (`'continuous' | 'nightly' | 'weekly' | 'quarterly' | 'adhoc'`), `review_role` (text nullable), `worldview_enabled` (boolean — toggling false stops worldview compile triggers without losing the body), `llm_budget_monthly_cap_usd` (numeric nullable). `DomainDetail.tsx` Configuration section adds the five matching field rows + per-field clear semantics consistent with the API. W3-followup reverts the Configuration-section `pl.json` keys back to English to match the rest of the placeholder-locale convention + expands the Drizzle `UPDATE` block comment to document the `${value}` parameterisation guarantee.

- **Source-binding retention override + notes editors** (W5 API `983a41b` / #145 · W5-UI `1ff3649` / #151). Two new PATCH branches on `/api/admin/source-bindings/:id`: `retention_days_override` (1–365 nullable; null reverts to the domain's `retention_days`) + `notes` (max 4096 chars, nullable; the freeform display-label text the BindingRow already convention-uses). SourceBindingDetail drill-down ships matching `RetentionOverridePanel` + `NotesPanel` editors with the wave-12 toast pattern. New audit verbs: `source_binding.set_retention_override`, `source_binding.set_notes`.

- **Output-channel bulk-delete** (W6 `4a4bf19` / #142). New `POST /api/admin/output-channels/bulk-delete` accepts `{ids: string[]}` and returns `{deleted, skipped}` so the UI can render "3 deleted, 2 already gone" without re-querying. Outputs tab gains left-most row checkboxes + select-all header checkbox + `Delete N` button gated by the ack-checkbox destructive-confirm modal pattern from `DomainDetail`. Audit invariant: one `output_channel.delete` row written PER id BEFORE the row's DELETE (per-item audit-write-before-mutate — operators triaging a batch deletion read each individual row, not a wrapping bulk verb).

- **Reports diagnostic surface + empty-state UX** (W8 `381d426` / #148). New `GET /api/admin/heartbeat/preconditions` counts heartbeat instances (total / enabled / channel-less) + surfaces the newest `agent_runs` row with status + `outputIsNull` flag (read-only, admin-team gated, no run output bytes exposed). The silent "no heartbeats yet" notice replaced by `HeartbeatDiagnosticsPanel` which walks the precondition chain top-to-bottom (no instance → disabled → no channels → no runs → `output IS NULL` → failed → healthy) and renders the first miss with an inline CTA deep-linking to Agents / Activity. Heartbeat + redaction `catch` paths now wrap a UI-local `safeErrorMessage` helper mirroring `@opencoo/shared/scrub`'s contract — operators see "HTTP 504" or "401 Unauthorized" instead of a generic "Error", with Bearer-token bytes scrubbed defensively. Three new pin-tests on `routes/heartbeat.ts:46-62` lock the `output IS NULL` exclusion + `DISTINCT ON (instance_id)` newest-per-group + inter-instance ordering assumptions.

- **Shared `TextField` / `TextArea` / `Table` primitives + i18n cleanup** (W9 `d4ae103` / #150). New `packages/ui/src/components/TextField.tsx` + `TextArea.tsx` thin controlled-string wrappers over `Field`'s discriminated union — `(next: string) => void` handler keeps call sites free of `ChangeEvent` boilerplate. New `Table.tsx` props-driven shared primitive (per-cell style overlays + per-row `<tr>` attribute callbacks + `renderAfterRow` for accordion drill-downs) replaces three open-coded `<table>` instances on `Reports.tsx:395-496`, `Cost.tsx`, `Audit.tsx`. Reports redaction column headers and Audit `ip:` / `ua:` prefixes lifted to i18n keys; `formatUsd()` in `Cost.tsx:143-157` now accepts `i18n.language` so the locale tag drives `Intl.NumberFormat` instead of being hardcoded `en-US`.

- **Sidebar groups + breadcrumbs + Cmd-K command palette** (W10 `788de22` / #153). The flat 11-tab sidebar becomes four named clusters with mono-uppercase micro-labels: **Operate** (Agents · Outputs · Activity) · **Knowledge** (Domains · Sources · Prompts) · **Governance** (Review · LlmPolicy · Cost · Audit) · **Diagnostics** (Reports). Operate first to give daily-task surfaces primacy. TopBar replaces the bare uppercase title with a `<group> / <tab> [ / <row-name>]` breadcrumb (Domains/Sources/Agents lift their selected row's label into App state via `onCrumbChange` so the deeper crumb renders during drill-down). New `CommandPalette.tsx` (~360 LOC, Cmd-K / Ctrl-K toggle gated on `authed`) fetches admin-API rows for domains + source-bindings + agent-instances + the hard-coded prompt-name roster on open, scores them with a substring + prefix matcher (prefix matches outrank mid-string, stable ties on original list order), and dispatches a `(tab, entityId?, promptName?)` navigation target. Read-only by contract — never mutates, only dispatches navigation callbacks. Domains/Sources/Agents grew an `initialOpenId` prop so palette-dispatched navigation auto-opens the matching drill-down once rows resolve.

- **Design-system audit pass** (W11 `02e0af6` / #155). Sweep of every UI route file for `design_system/README.md` "Hard nos" + accent-color budget violations. Real fix: `CommandPalette` was tinting bold match-highlight characters with `var(--wiki)` — the palette spans non-knowledge entities (domains / agents / sources / prompts) so wiki-teal violated the "Wiki Teal only on compiled-knowledge chrome" rule; switched to `var(--ink)` + `fontWeight: 600`. Hard-no sweeps came up clean: zero gradients, zero box-shadows, zero `backdrop-filter` in code (all docstring guard comments), all radii ≤ 6px on cards / ≤ 10px on sheets (sheet cap respected by `CommandPalette` / `DiffPreviewDialog` / `Modal` / `PatEntryModal` only), zero emoji in user-facing copy + locale files (one stray U+2713 inside an `AgentsRunNowButton.tsx` JSDoc fixed to prose), zero marketing-voice strings in copy. Heartbeat-pulse remains the only `infinite` animation; all others (`opencoo-dialog-enter`, `opencoo-field-enter`, `opencoo-row-slide-out-up`, `opencoo-banner-fade-out`) are one-shot `forwards` with `var(--ease-write)` / `var(--ease-transform)`. Accent-color budget audit-comments embedded as PR-W11 stamps in each route's top docstring for the next audit cycle.

### Deferred to wave-16 / v0.2

- **W11 audit residuals (zero behavioural, doc-only).** The audit found one real fix (CommandPalette match-highlight color), which W11 already shipped. The remaining "findings" are all confirmations that the sweep came up clean — they're recorded as PR-W11 stamps in each route's top docstring so the next audit cycle has a fixed reference, but no further code action falls out of W11.
- **Cleanup pipeline reading `sources_bindings.retention_days_override`.** W5 ships the column + API + UI so operators can SET the override, but the existing weekly Cleanup pipeline still reads `domains.retention_days` only — the per-binding override is honored at admin-API write-time but not yet at sweep-time. v0.2 — extend the Cleanup pipeline's per-binding age query to `COALESCE(b.retention_days_override, d.retention_days)`. Documented as explicit deferral in the W5 PR body.
- **Rich-text / Monaco prompt editor.** W7a + W7b ship a plain `<textarea>` for the override body. Operators editing 200-line prompts get monospace + a baseline-version diff in the apply dialog, but no syntax highlighting / no inline diff against the override version they last applied / no soft-wrap toggle. v0.2 once an operator workflow surfaces the gap.
- **3-way merge UI for stale overrides.** When a shipped baseline rev lands while an override is on disk, the W2 baseline-version-bound sovereignty token rejects the apply with 422 and the operator re-previews against the new baseline. The current UI flow shows the new diff but doesn't show what changed in the shipped baseline between the two revs — for prompts the override and the baseline have diverged on, this is a manual `git log` exercise. v0.2 — a `BaselineDiffViewer` panel showing baseline rev N → N+1 alongside the override.
- **Per-prompt rate-limit / budget knobs.** `domains.llm_budget_monthly_cap_usd` (W3) caps the whole domain's monthly spend; prompts that an override pushes into a more-expensive tier (e.g. a per-instance heartbeat override that switches `Worker → Thinker`) have no per-prompt ceiling. v0.2 — `prompt_overrides.tier_cap` column + cost-tracker enforcement at run-start.
- **Single-use sovereignty tokens within TTL.** The W2 sovereignty token is bound to (scope, payload-hash, 5-min TTL) and a baseline rev — it accepts multiple apply calls within the TTL window with the same payload-hash, which matches the wave-12 `domains-llm-policy.ts` pattern but doesn't prevent a malicious replay if the cookie is compromised within 5 minutes. v0.2 — single-use semantics enforced via a `prompt_override_tokens` table with INSERT-then-DELETE on apply.
- **Concurrent-edit locking on prompt overrides.** Two operators editing the same `(domain_id, prompt_name, locale)` row in two tabs both succeed last-write-wins. The W2 preview ↔ apply baseline-version binding catches the "baseline shipped between your preview and your apply" race but not the "two operators applied within the same baseline window" race. v0.2 — `prompt_overrides.lock_token` + an explicit `If-Match` check.

### Risk residual

- **Empty-database boot now lights up the Reports tab diagnostic panel.** Before W8 a fresh deploy showed "no heartbeats yet" on Reports forever; after W8 the panel names the first missing precondition (no instance → disabled → no channels → no runs → `output IS NULL` → failed → healthy) with a deep-link CTA. An operator who hits the page on a freshly-provisioned deployment now reads a chain of remediation steps instead of silence — fewer "is the page broken?" tickets, but it does mean the Reports tab now has more on-screen verbiage on day 0 than it does on day N.
- **Operator-edited prompts bypass the deterministic injection corpus.** The injection-corpus harness assembles prompts against the **shipped baselines** at fixture-generation time — `pnpm test:injection` continues to assert that the shipped baseline + sentinel→amp→xmlbody ordering survives the 88-fixture sweep, but an operator override that loosens a `<source_content>` boundary or removes a redaction-defense paragraph is not corpus-tested. Documented at `packages/shared/src/prompts/loadPromptForScope.ts` — the `loadPrompt({name, locale})` deterministic loader the corpus uses stays unchanged; only the per-(domain, instance) runtime resolver reads from `prompt_overrides`. Operators editing prompts inherit responsibility for not re-introducing prompt-injection surface; the W7a / W7b dialogs reference this in their docstring.
- **Audit-write-before-mutate invariant pinned across all new wave-15 verbs.** Every new admin-API write path follows the `auditWrite() → mutate → return` ordering (matching the wave-12 `domains-llm-policy.ts` pattern), with one audit row per atomic mutation: `prompt_override.apply` / `prompt_override.delete` (W2), `agent_instance.create` / `agent_instance.set_scope` / `agent_instance.set_name` / `agent_instance.set_locale` / `agent_instance.memory_clear` (W4 API), `source_binding.set_retention_override` / `source_binding.set_notes` (W5 API), `domain.update` extended to cover the new five-field PATCH branches (W3), `output_channel.delete` written PER id BEFORE the row's DELETE on bulk-delete (W6). The invariant survives the wave; operators triaging an incident across the new surfaces read one audit row per individual change, not a wrapping bulk verb.

Wave-15 ships under `0.1.0-a.13` once this PR (W12) merges.
...
- **Per-row Retry button wiring + `(N)` count on the binding-scoped Retry button.** Two related v0.2 polish items on `SourceBindingDetail`: (a) W4 ships the recent-failed-rows list with a per-row Retry button parked disabled + tooltip pointing at W2 (W2's endpoint accepts the `?intakeId=...` single-job scope — only the SPA caller wire-up + the small per-row in-flight UX remain), and (b) the binding-scoped "Retry failed jobs" button label ships without the `(N)` count because the count source landed in W4's GET response after W2 had merged; once a small follow-up surfaces `intakeCounts.failed` into the button's label render, the label becomes "Retry failed jobs (N)" and the disabled-when-N=0 affordance lands. Backend is ready in both cases.
